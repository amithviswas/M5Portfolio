...
const interactiveElementHoverClass = "hover:translate-y-[-1px] hover:shadow-[0_2px_8px_hsl(var(--primary)/0.5),_0_2px_8px_hsl(var(--bmw-m-blue)/0.5)] transition-all duration-200 ease-out";
...
export default function ContactPage() {
  ...
  const onSubmit: SubmitHandler<IFormInput> = async (data) => {
    ...
    toast({
      ...
      variant: "default", // Success variant could be added to globals.css
    });
    reset();
  };

  return (
    <Section id="contact-page" className="min-h-screen !py-12 md:!py-16"> {/* Reduced padding for full effect */}
      ...
      <div className="grid md:grid-cols-12 gap-8 md:gap-10 items-start">
        {/* Left Column: Digital Dash Panel */}
        <motion.div
            ...
            <div className="flex flex-col space-y-4">
              {socialLinks.map((link) => (
                <Link key={link.name} href={link.href} target="_blank" rel="noopener noreferrer" aria-label={link.ariaLabel}
                  className={cn(
                    "group w-full p-3 rounded-full bg-neutral-800 hover:bg-neutral-700 border border-neutral-700 transition-all duration-200 ease-out",
        ...
        <motion.div
          className="md:col-span-7 bg-gradient-to-b from-neutral-800/70 to-neutral-900/70 p-6 md:p-8 rounded-lg shadow-xl card-m-glow backdrop-blur-sm border border-border/30"
          initial={{ opacity: 0, x: 50 }}
          animate={{ opacity: 1, x: 0 }}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              ...
              <Input
                type="text"
                id="name"
                className={cn("mt-2 bg-background/70 border-border/50 focus:border-primary focus:ring-primary placeholder:text-muted-foreground/70", errors.name && "border-destructive focus:border-destructive ring-destructive")}
            ...
            <div>
              ...
              <Input
                type="email"
                id="email"
                className={cn("mt-2 bg-background/70 border-border/50 focus:border-primary focus:ring-primary placeholder:text-muted-foreground/70", errors.email && "border-destructive focus:border-destructive ring-destructive")}
            ...
            <div>
              ...
              <Textarea
                id="message"
                rows={5}
                className={cn("mt-2 bg-background/70 border-border/50 focus:border-primary focus:ring-primary placeholder:text-muted-foreground/70", errors.message && "border-destructive focus:border-destructive ring-destructive")}
            ...
            <div className="pt-2 flex justify-center">
              <Button 
                type="submit" 
                className={cn(
                  "w-32 h-32 rounded-full bg-primary hover:bg-primary/80 text-primary-foreground text-xl font-bold uppercase tracking-wider shadow-lg hover:shadow-primary/60 transition-all duration-300 ease-in-out focus:outline-none focus:ring-4 focus:ring-primary/50 focus:ring-offset-2 focus:ring-offset-background",
                  interactiveElementHoverClass
                )} 
