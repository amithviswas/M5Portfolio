...
export default function HeroSection() {
  ...
  const handleStartDriveClick = useCallback(() => {
    setIntroCompleted(true); // Mark intro as done
    // Ensure DOM update from setIntroCompleted has a chance to process
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (heroSectionHeight > viewportHeight) {
         // Calculate scroll percentage based on hero section height relative to viewport
         scrollPercent = Math.min(100, (currentScrollY / (heroSectionHeight - viewportHeight)) * 100);
      } else if (currentScrollY > 0) {
         // If hero is shorter than viewport, any scroll means 100% for tachometer purpose
         scrollPercent = 100;
      }
  ...
  const containerVariants = {
    ...
    animate: {
      opacity: 1,
      transition: { staggerChildren: 0.15, delayChildren: introCompleted ? 0.2 : 0.2 } // Stagger for children on initial load after intro
    },
  };

  const itemVariants = {
    medallion: heroTextAnimation(introCompleted ? 0.2 : 0.2, 0.5),
    headline: heroTextAnimation(introCompleted ? 0.4 : 0.4, 0.7),
    tagline: heroTextAnimation(introCompleted ? 0.55 : 0.55, 0.7),
    paragraph: heroTextAnimation(introCompleted ? 0.8 : 0.8, 0.6, 10), // Slightly less yOffset for paragraph
    // Button variant removed
  };
  
  const getTachometerBarGradient = (percentage: number) => {
    const blue = 'hsl(var(--empire-i-blue-fog-start-hsl))'; // Use the i-blue fog for start
    const royalBlue = 'hsl(var(--empire-royal-blue-hsl))';
    const crimson = 'hsl(var(--empire-crimson-hsl))';
    const gold = 'hsl(var(--empire-gold-hsl))';

    if (percentage <= 40) return `linear-gradient(to right, ${blue}, ${royalBlue})`;
    if (percentage <= 80) return `linear-gradient(to right, ${royalBlue}, ${crimson})`;
    return `linear-gradient(to right, ${crimson}, ${gold})`;
  };
    ...
    >
      ...
      >
        ...
        <motion.h1
          ...
          className={cn(
            "text-5xl sm:text-6xl md:text-7xl text-primary-foreground hero-headline-empire relative", 
            headlineBeamFrozen && !prefersReducedMotion && "beam-frozen", 
            isHeadlineHovered && !prefersReducedMotion && "animate-cameraShutterFlash"
          )}
          style={{ opacity: headlineOpacity }} // Apply scroll-based opacity
          onMouseEnter={() => setIsHeadlineHovered(true)}
          onMouseLeave={() => setIsHeadlineHovered(false)}
        >
          Coded Precision.
        ...
        <motion.h2
          ...
          className={cn(
            "hero-tagline-empire text-xl md:text-2xl mt-2 md:mt-3 font-semibold uppercase tracking-wider md:tracking-widest", 
            isTaglineUnderlineVisible && "underline-visible"
          )}
          style={{ opacity: taglineOpacity }} // Apply scroll-based opacity
        >
        ...
        <motion.p
          ...
          className="mt-6 md:mt-8 max-w-xl mx-auto text-base text-foreground/80 sm:text-lg font-inter" // Changed to font-inter
        >
        ...
        {/* MStartStopButton removed from here */}
      ...
      {!prefersReducedMotion && introCompleted && (
        <motion.div
          ...
          animate={{opacity:1, y:0, transition: {delay: 0.5, duration:0.5}}} // Animate in after main content
        >
          <motion.div
          ...
          />
          {/* Optional: Trail effect for tachometer bar */}
          <motion.div
            className="hero-empire-tachometer-trail"
            style={{ width: `${scrollPercentForTachometer}%`, opacity: scrollPercentForTachometer > 5 ? 0.5 : 0 }} // Show trail if bar has some width
          />
        </motion.div>
      )}
    </Section>
