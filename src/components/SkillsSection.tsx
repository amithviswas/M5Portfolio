...
import {
  Brain, Database, BarChart3, Cpu, Code2, Bot, Palette, Terminal, Server, Zap, Wind, Gauge // Added Zap, Wind, Gauge
} from 'lucide-react';
import Section from '@/components/Section';

// Updated skills to "Engine Modes"
const skills = [
...
const cardVariants = {
  ...
  hover: {
    ...
    boxShadow: "0px 0px 20px 0px hsl(var(--primary)/0.6)", // M-glow effect
    borderColor: "hsl(var(--primary))", // Glowing border
...
const iconVariants = {
  hover: {
    scale: 1.15,
    color: "hsl(var(--primary))", // Icon glows red
...
export default function SkillsSection() {
  return (
    <Section id="skills" className="bg-card/30">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-4 md:gap-6"> {/* Adjusted lg:grid-cols */}
        {skills.map((skill, index) => (
          <motion.div
            key={skill.modeName}
            className="bg-card/70 border border-border/30 rounded-lg p-4 md:p-6 text-center cursor-default shadow-lg hover:shadow-primary/40" // Enhanced shadow
            custom={index}
          ...
          >
            ...
            <motion.p
              ...
              initial={{ opacity: 0 }} // Hidden by default, shows on hover
            >
              ({skill.name})
            </motion.p>
          </motion.div>
